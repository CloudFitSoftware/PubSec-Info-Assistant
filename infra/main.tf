locals {
  ...
  abbr            = jsondecode(file("${path.module}/abbreviations.json"))
...
module "backend" {
  ...
  appSettings = {
    ...
    TARGET_EMBEDDINGS_MODEL                 = var.useAzureOpenAIEmbeddings ? "azure-openai_${var.azureOpenAIEmbeddingDeploymentName}" : var.sentenceTransformersModelName
    ENRICHMENT_APPSERVICE_URL               = module.enrichmentApp[0].uri
...
# // Function App 
module "functions" { 
  // count           = var.isContainerizedDeployment ? 0 : 1 using one block with specific logic instead of breaking up into two blocks
  ...
  azureSearchServiceEndpoint            = var.isContainerizedDeployment ? "" : module.searchServices[0].endpoint
  endpointSuffix                        = var.azure_storage_domain
  azure_ai_text_analytics_domain        = var.azure_ai_text_analytics_domain
  azure_ai_translation_domain           = var.azure_ai_translation_domain
  isContainerizedDeployment             = var.isContainerizedDeployment
  acrEndpoint                           = var.isContainerizedDeployment ? module.acr[0].endpoint : ""
  imageName                             = var.isContainerizedDeployment ? "function" : ""
  imageTag                              = var.isContainerizedDeployment ? "latest": ""
...
module "openAiRoleBackend" {
  ...
  principalId      = module.backend[0].identityPrincipalId
...
module "aks" {
  ...
  source          = "./core/compute"
  aksIdentityType = var.aksIdentityType
  aksSkuTier      = var.aksSkuTier
  aksSystemVmSize = var.aksSystemVmSize
  aksUserVmSize   = var.aksUserVmSize
  clusterName     = "infoasst-${local.abbr.containerServiceManagedClusters}${random_string.random.result}"
  keyVaultName    = module.kvModule.keyVaultName
  keyVaultId      = module.kvModule.keyVaultId
  location        = var.location
  resourceGroupName = azurerm_resource_group.rg.name
...
module "acr" {
  count             = var.isContainerizedDeployment ? 1 : 0
  source            = "./core/storage/acr"
  name              = "infoasst${local.abbr.containerRegistryRegistries}${random_string.random.result}"
  acrSku            = var.acrSku
  location          = var.location
  resourceGroupName = azurerm_resource_group.rg.name
...
module "NetworkContributorRole" {
  ...
  source             = "./core/security/role"
  scope              = azurerm_resource_group.rg.id
  principalId        = module.aks[0].aksPrincipalId
  principalType      = "ServicePrincipal" # Default is set in the module, change if necessary
  roleDefinitionId   = local.azure_roles.NetworkContributor
  subscriptionId     = var.subscriptionId
  resourceGroupId    = azurerm_resource_group.rg.id 
  depends_on         = [module.aks]
}
